...
public final class SocksCmdResponse extends SocksResponse {
	...
	// All arrays are initialized on construction time to 0/false/null remove
	...
	/**
	 * Constructs new response and includes provided host and port as part of
	 * it.
	 *
	 * @param cmdStatus
	 ...
	 * @param host
	 *            host (BND.ADDR field) is address that server used when
	 *            connecting to the target host. When null a value of 4/8 0x00
	 *            octets will be used for IPv4/IPv6 and a single 0x00 byte will
	 *            be used for domain addressType. Value is converted to ASCII
	 *            using {@link IDN#toASCII(String)}.
	 * @param port
	 *            port (BND.PORT field) that the server assigned to connect to
	 *            the target host
	 * @throws NullPointerException
	 *             in case cmdStatus or addressType are missing
	 * @throws IllegalArgumentException
	 *             in case host or port cannot be validated
	 ...
	 * {@link io.netty.handler.codec.socks.SocksCmdType}. Host (BND.ADDR field
	 * in response) is address that server used when connecting to the target
	 * host. This is typically different from address which client uses to
	 * connect to the SOCKS server.
	 *
	 * @return host that is used as a parameter in
	 *         {@link io.netty.handler.codec.socks.SocksCmdType} or null when
	 *         there was no host specified during response construction
	 */
	public String host() {
		if (host != null) {
	 ...
	 * {@link io.netty.handler.codec.socks.SocksCmdType}. Port (BND.PORT field
	 * in response) is port that the server assigned to connect to the target
	 * host.
	 *
	 * @return port that is used as a parameter in
	 *         {@link io.netty.handler.codec.socks.SocksCmdType}
	 */
	public int port() {
		return port;
	}
	...
	public void encodeAsByteBuf(ByteBuf byteBuf) {
		if (protocolVersion() == SocksProtocolVersion.SOCKS4a) {
			byteBuf.writeByte(0x00);
			if(cmdStatus == SocksCmdStatus.SUCCESS){
				byteBuf.writeByte(0x5a);
			}else{
				byteBuf.writeByte(0x5b);
			}
